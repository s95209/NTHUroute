...
class NVR_Point3D {
  public:
    NVR_Point3D(unsigned x, unsigned y, unsigned z) : m_x(x), m_y(y), m_z(z) {
    }
...
class NVR_Net {
  public:
    ...
    const std::vector<NVR_Pin> &pins() const {
        return m_pins;
    }

    NVR_Box &box() {
        return const_cast<NVR_Box &>(const_cast<const NVR_Net *>(this)->box());
    }
    const NVR_Box &box() const {
        return m_box;
    }
...
class NVR_Gcell {
  public:
    void incr_demand(unsigned demand) {
        m_demand += demand;
    }
    void decr_demand(unsigned demand) {
        m_demand -= demand;
    }
    void set_demand(unsigned demand) {
        m_demand = demand;
    }
...
class NVR_GridGraph2D {
  public:
    ...
    bool is_routing_layer() const {
        return m_dir != NVR_DIR_NONE;
    }
    bool is_hor() const {
        return m_dir == NVR_DIR_HORIZONTAL;
    }
    bool is_ver() const {
        return m_dir == NVR_DIR_VERTICAL;
    }

    void set_min_length(double min_length) {
    ...
    double unit_length_cost() const {
        return m_unit_length_cost;
    }
    double unit_via_cost() const {
        return m_unit_via_cost;
    }
    double unit_overflow_cost() const {
        return m_unit_overflow_cost;
    }
...
void NVR_GridGraph2D::set_unit_cost(double length, double via, double overflow) {
...
class NVR_GridGraph {
  public:
    unsigned num_gridx() const {
        return m_num_gridx;
    };
    unsigned num_gridy() const {
        return m_num_gridy;
    };
    unsigned num_layer() const {
        return m_num_layer;
    };
    void init(unsigned x, unsigned y, unsigned z);
    NVR_GridGraph2D &plane(unsigned layer) {
        return m_plane[layer];
    }
    const NVR_GridGraph2D &plane(unsigned layer) const {
        return m_plane[layer];
    }

    void init_x_coords(std::vector<int> &coord) {
        m_x_coords = coord;
    }
    void init_y_coords(std::vector<int> &coord) {
        m_y_coords = coord;
    }
    int cell_width(int x) const {
        return m_x_coords[x + 1] - m_x_coords[x];
    }
    int cell_height(int y) const {
        return m_y_coords[y + 1] - m_y_coords[y];
    }
...
class NVR_DB {
  ...
  private:
    ...
    bool check_connectivity(const NVR_Net *net, std::vector<std::vector<std::vector<int>>> &flag) const;
...
bool NVR_DB::read_nets(const char *input) {
    ...
    while (std::getline(net_file, line)) {
        if (line.find("(") == std::string::npos && line.find(")") == std::string::npos
            && line.length() > 1) { // start to read a net
            ...
            line.erase(
                std::remove_if(line.begin(), line.end(),
                               [&redundant_chars](char c) { return redundant_chars.find(c) != std::string::npos; }),
                line.end());
...
#ifdef DEBUG
    std::fstream debug_file(debug_argv, std::ios::out | std::ios::trunc);
#endif
        ...
        for (unsigned x = 0; x < m_graph.num_gridx(); x++) {
            for (unsigned y = 0; y < m_graph.num_gridy(); y++) {
                ...
                if (z >= 0 && z <= 2) {
                    if (overflow > 0) {
                        debug_file << "(" << z << ", " << x << ", " << y << ") : "
                                   << "demand (" << demand << ")= 2* wire_cnt (" << wire_counter[z][x][y]
                                   << ") + nonstackVia_cnt (" << nonstack_via_counter[z][x][y]
                                   << "), capacity= " << cell.capacity() << ", num_overflows= " << overflow
                                   << std::endl;
                    }
                }
#endif
                ...
                if (gg.is_hor()) {
                    layer_wl += wire_counter[z][x][y] * m_graph.cell_width(x);
                }
                else if (gg.is_ver()) {
                    layer_wl += wire_counter[z][x][y] * m_graph.cell_height(y);
                }
            }
        }
...
#ifdef IDEAL
    printf("\n------ The above result is under the condition of 'Demand = 0! (IDEAL)' ------\n\n");
#endif

    return true;
}

void NVR_DB::update_nonstack_via_counter(unsigned net_idx, const std::vector<NVR_Point3D> &via_loc,
                                         std::vector<std::vector<std::vector<int>>> &flag,
                                         std::vector<std::vector<std::vector<int>>> &nonstack_via_counter) const {
...
bool NVR_DB::check_connectivity(const NVR_Net *net, std::vector<std::vector<std::vector<int>>> &flag) const {
    int mark = net->idx();
