...
class Multisource_multisink_mazeroute
{
    ...
    private:
	void setup_pqueue(vector<MMM_element*> &sinks);
	void find_subtree(Vertex_mmm *v, int mode, vector<MMM_element*> &sinks);
    ...
    //Gamer
    bool gamer_p2(vector<MMM_element*> &sinks);
    bool gamer_p3(vector<MMM_element*> &sinks);
    void row_sweep_p2(int row);
    void col_sweep_p2(int col);
    void row_sweep_p3(int row);
    void col_sweep_p3(int col);
    MMM_element* find_min_cost_sink(vector<MMM_element*> &sinks);
    ...
    int min_l, min_b, max_r, max_t;

    double bound_cost;
    int bound_via, bound_dist;
    // struct Padded {
    //     double bound_cost;
    //     char pad1[56];
    //     int bound_via;
