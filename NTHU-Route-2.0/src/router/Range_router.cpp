...
void define_interval()
{
    ...
    if(max_congestion >= 2) max_congestion_factor = 2.0;
...
static void insert_to_interval(double cong_value,Coordinate_2d* coor_2d, int dir)
{
	...
	{
        if ( ( (cong_value < interval_list[i].begin_value) ||
...
static void query_range_2pin(int left_x,
                             int bottom_y,
                             int right_x,
                             int top_y,
                             vector<Two_pin_element_2d *> *twopin_list)
{
	...
	{
		for(vector<Two_pin_element_2d*>::iterator it = cell_list[i]->points.begin();
            it != cell_list[i]->points.end();
            ++it)	//for each pin or steiner point
		{
