...
inline std::string viaToGuide(int x, int y, int layer1, int layer2)
{
	...
	for (int l = layer_1; l <= layer_2; ++l) {
		str += std::to_string(x) + " " + std::to_string(y) + " " + std::to_string(x+1) + " " + std::to_string(y+1) + " metal" + std::to_string(l) + "\n";
	}
	return str;
	...
	for(i = 1; i < max_xx; ++i)
		for(j = 0; j < max_yy; ++j)
		{
			...
			for(k = 0; k < max_zz; ++k)
			{
				...
				{
					// printf("%d %d %d, cur_cap: %d, max_cap: %d\n", i, j, k, cur_map_3d[i][j][k].edge_list[LEFT]->cur_cap, cur_map_3d[i][j][k].edge_list[LEFT]->max_cap);
		...
		for(j = 1; j < max_yy; ++j)
		{
			...
			for(k = 0; k < max_zz; ++k)																						                          
			{
				...
				if (cur_map_3d[i][j][k].edge_list[BACK]->cur_cap > cur_map_3d[i][j][k].edge_list[BACK]->max_cap)	// overflow
				{
					...
					sum_overflow[k] += (cur_map_3d[i][j][k].edge_list[BACK]->cur_cap - cur_map_3d[i][j][k].edge_list[BACK]->max_cap);
					lines++;
				}
			}
		}
...
void initial_overflow_map(void)
{
	...
	for(i = 0; i < max_xx; ++i) 
		for(j = 1; j < max_yy; ++j) {
			*(overflow_map[i][j].edge[BACK]) = (congestionMap2d->edge(i, j, DIR_SOUTH).overUsage()); // modified
			// if (i == 94 && j == 112) {
			// 	printf("%d %d\n", *(overflow_map[i][j].edge[BACK]), (congestionMap2d->edge(i, j, DIR_SOUTH).overUsage()));
			// 	printf("%lf %lf\n", congestionMap2d->edge(i, j, DIR_SOUTH).cur_cap, congestionMap2d->edge(i, j, DIR_SOUTH).max_cap);
			// 	printf("%lf\n", congestionMap2d->edge(i, j, DIR_SOUTH).cur_cap-congestionMap2d->edge(i, j, DIR_SOUTH).max_cap);
				
			// 	assert(*(overflow_map[i][j].edge[BACK]) == *(overflow_map[i][j-1].edge[FRONT]));
			// 	assert(round(congestionMap2d->edge(i, j, DIR_SOUTH).cur_cap-congestionMap2d->edge(i, j, DIR_SOUTH).max_cap) == 1);
			// }
	...
	// printf("(94, 111) congestionMap2d cur_cap: %lf, max_cap: %lf\n", congestionMap2d->edge(94, 111, DIR_NORTH).cur_cap, congestionMap2d->edge(94, 111, DIR_NORTH).max_cap);
}
...
void update_cur_map_for_klat_xy(int cur_idx, Coordinate_2d *start, Coordinate_2d *end, int net_id)
{
	...
	if (start->x != end->x && start->y == end->y)
	{
		...
		cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->cur_cap = (cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->used_net.size()); // modified
		...
		{
			// printf("%d %d %d, cur_cap: %lf, max_cap: %lf\n", end->x, end->y, cur_idx, cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->cur_cap, cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->max_cap);
	...
	else if (start->y != end->y && start->x == end->x)
	{
		...
		{
			// printf("%d %d %d, cur_cap: %d, max_cap: %d\n", end->x, end->y, cur_idx,
			// cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->cur_cap, cur_map_3d[end->x][end->y][cur_idx].edge_list[dir_idx]->max_cap);
			// if (dir_idx == BACK)
			// 	printf("S: cur_cap: %lf,  max_cap: %lf\n", congestionMap2d->edge(end->x, end->y, DIR_SOUTH).cur_cap, congestionMap2d->edge(end->x, end->y, DIR_SOUTH).max_cap);
			// else
			// 	printf("N: cur_cap: %lf,  max_cap: %lf\n", congestionMap2d->edge(end->x, end->y, DIR_NORTH).cur_cap, congestionMap2d->edge(end->x, end->y, DIR_NORTH).max_cap);
			*(overflow_map[end->x][end->y].edge[dir_idx]) -= 1; // modified
			assert(*(overflow_map[end->x][end->y].edge[dir_idx]) >= 0);
		}
	}
	...
	if (level < 4)
	{
			...
			if (path_map[global_x][global_y].edge[level] == 1)
			{
				...
				for(k = 0; k < global_max_layer; ++k)	// use global_max_layer to substitue max_zz
					...
					{
						std::cout << "x = " << temp_x << " y = " << temp_y << " k = " << k << " val = " << klat_map[temp_x][temp_y][k].val << std::endl;
						temp_idx[level] = k;
				...
				if (*(overflow_map[x][y].edge[i]) <= 0){	// this edge could not happen overflow
					for(k = 1; k < global_max_layer; ++k)	// use global_max_layer to substitute max_zz 
					{
						if (cur_map_3d[x][y][k].edge_list[i]->cur_cap < cur_map_3d[x][y][k].edge_list[i]->max_cap) {
							assigned = true;
							// if (i == 0 || i == 1) {
							// 	// std::cout << cur_map_3d[x][y][k].edge_list[i]->cur_cap << ' ' << cur_map_3d[x][y][k].edge_list[i]->max_cap << std::endl;
							// 	assert(database.getLayerDir(k) == X);
							// }
							// if (i == 3 || i == 4)
					...
					for(k = 1; k < global_max_layer; ++k)	// use global_max_layer to substitute max_zz
					{
						if ((cur_map_3d[x][y][k].edge_list[i]->cur_cap - cur_map_3d[x][y][k].edge_list[i]->max_cap) < overflow_max) {	// overflow, but doesn't over the overflow_max
							...
							// std::cout << "cur_cap: " << cur_map_3d[x][y][k].edge_list[i]->cur_cap << ", max_cap: " << cur_map_3d[x][y][k].edge_list[i]->max_cap << std::endl;
							if ((i == 0 || i == 1) && k % 2 == 1) {
								DP(temp_x, temp_y, k);
								assigned = true;
							}
							else if ((i == 2 || i == 3) && k % 2 == 0) {
								DP(temp_x, temp_y, k);
								assigned = true;
							}
							// DP(temp_x, temp_y, k);
		...
		if (is_end == true)
		{
			if (is_metal5)
			{
				...
				for (k = z, klat_map[x][y][z].via_overflow = 0; k < path_map[x][y].max_pin_layer; ++k) {
					...
					for (k = path_map[x][y].min_pin_layer, klat_map[x][y][z].via_overflow = 0; k < path_map[x][y].max_pin_layer; ++k) {
						if (viadensity_map[x][y][k].cur >= viadensity_map[x][y][k].max)
							++klat_map[x][y][z].via_overflow;
					}
				}
	...
	//# Q change via overflow l k <= z -> k < z
		...
		else	// is_end == false
		{
			...
			if(check_net){
				...
				std::cout << "global.x = " << global_x << " global.y = " << global_y << " global.z = " << min_DP_k << std::endl;
			...
			for(i = 0; i < 4; ++i)	// direction
				if (min_DP_idx[i] >= 0)
				{
					...
					temp_y = y + plane_dir[i][1];
					if(check_net){
						std::cout << "x = " << temp_x << " y = " << temp_y << " k = " << min_DP_idx[i] << " via_cost = " << klat_map[temp_x][temp_y][min_DP_idx[i]].via_cost << std::endl;
					}
...
void generate_output(int net_id, std::ofstream &fout)
{
	...
	// 	// printf("Pin: (%d, %d, %d)\n", pin->get_tileX(), pin->get_tileY(), pin->get_layerId());
	// 	// printf("%d %d %d %d %d %d\n", pin->get_tileX(), pin->get_tileY(), pin->get_layerId()+1, pin->get_tileX(), pin->get_tileY(), pin->get_layerId()+1);
	...
	q.push(&coord_3d_map[(*pin_list)[0]->get_tileX()][(*pin_list)[0]->get_tileY()][(*pin_list)[0]->get_layerId()]);	// enqueue
	// if (rr_map->get_net_name(net_id) == "pin1")
	// 	printf("Pin: (%d, %d, %d) %d\n", (*pin_list)[0]->get_tileX(), (*pin_list)[0]->get_tileY(), (*pin_list)[0]->get_layerId(), pin_count);
#else
	...
	for(int i = 0;i < pinnum ;i++){
		printf("(%d,%d,%d)\n",(*pin_list)[i]->get_tileX(),(*pin_list)[i]->get_tileY(),(*pin_list)[i]->get_layerId());
	}
#elif VERIFY_PIN_1
	int pinnum = rr_map->get_netPinNumber(net_id);
	for(int i = 0;i < pinnum ;i++){
			...
			if (i >= 2 || j >= 2)	// have edge
			{
				...
				if (start.z == end.z) {
					...
					if (start.x == end.x) {
						...
						if (length >= LENGTH) {
							for (int p = p_start; p<=p_end;) {
								...
								if (cur_map_3d[start.x][p][start.z-1].cap < THRESH) {
									// printf("%d %d %d %d %d %d\n", max(start.x-1, 0), p, start.z, min(start.x+1, rr_map->get_gridx()-1), p, start.z);
									fout << rectToGuide(max(start.x-1, 0), p, min(start.x+1, rr_map->get_gridx()-1), p, start.z);
									if (start.z-1 + 1 < max_zz && cur_map_3d[start.x][p][start.z-1 + 1].cap >= THRESH) {
										patched = true;
										// printf("%d %d %d %d %d %d\n", max(start.x-1, 0), p, start.z+1, min(start.x+1, rr_map->get_gridx()-1), p, start.z+1);
										fout << rectToGuide(max(start.x-1, 0), p, min(start.x+1, rr_map->get_gridx()-1), p, start.z+1);
									}
						...
						if (length >= LENGTH) {
							for (int p = p_start; p<=p_end;) {
								...
								if (cur_map_3d[p][start.y][start.z-1].cap < THRESH) {
									// printf("%d %d %d %d %d %d\n", p, max(start.y-1, 0), start.z+1, p, min(start.y+1, rr_map->get_gridy()-1), start.z+1);
									fout << rectToGuide(p, max(start.y-1, 0), p, min(start.y+1, rr_map->get_gridy()-1), start.z+1);
						...
						// 		printf("%d %d %d %d %d %d\n", p, max(start.y-1, 0), start.z, p, min(start.y+1, rr_map->get_gridy()-1), start.z);
						// 		if (start.z-1 + 1 < max_zz) {
						// 			printf("%d %d %d %d %d %d\n", p, max(start.y-1, 0), start.z+1, p, min(start.y+1, rr_map->get_gridy()-1), start.z+1);
						// 		}
						// 		// if (start.z-1 - 1 >= 1) {
...
void klat(int net_id)
{
	...
	if(check_net){
		...
		for(int i = 0; i< pin_list->size();i++){
			std::cout << i << ":" "x = " << (*pin_list)[i]->get_tileX() << " y = " << (*pin_list)[i]->get_tileY() << " z = " << (*pin_list)[i]->get_layerId() << std::endl;
		}
		const char *p;
		p = rr_map->get_netName(net_id);start_pin_layer
		std::cout << "net_id = " << net_id << "name = " << p << std::endl;
	}
...
void generate_all_output(std::ofstream &fout)
{
    ...
    for (auto &net : deletedNet)
    {
        ...
        {    
			...
			// 	fout << rectToGuide(std::max(x - 1, 0), std::max(y - 1, 0), std::min(x + 1, rr_map->get_gridx()), std::max(y - 1, 0), l);
			// 	fout << rectToGuide(std::max(x - 1, 0), y, std::min(x + 1, rr_map->get_gridx()), y, l);
			// 	fout << rectToGuide(std::max(x - 1, 0), std::min(y + 1, rr_map->get_gridy()), std::min(x + 1, rr_map->get_gridx()), std::min(y + 1, rr_map->get_gridy()), l);
